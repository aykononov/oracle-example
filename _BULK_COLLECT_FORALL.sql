...
Если в выборке всего несколько колонок, то каждую из них можно сохранить 
в отдельную переменную - коллекцию. 

Если нужно выбрать все колонки таблицы, то можно сохранить результат выборки в коллекции записей. 
Такая коллекция весьма удобна для циклического перебора результирующих записей, 
поля которых ссылаются на колонки таблицы.
...
Используя BULK COLLECT, можно улучшить код, выполняя больше работы в SQL:
Если надо пройти по результирующей выборке только один раз, то используем цикл FOR . 
Этот подход позволяет избежать выделение памяти на хранение копии результирующих данных.

Если из результирующих данных требуется выбрать определенные значения и поместить их в меньшую выборку, 
то используем фильтрацию в основном выражении. В простом случае используем условие WHERE . 

Для сравнения двух и более наборов данных лучше применить выражения INTERSECT и MINUS .

Если нужно циклически проходить по результирующей выборке и для каждого ряда выполнять DML-выражение 
или делать другую выборку, можно использовать более эффективных подход. Попробовать вложенную выборку переделать 
в подзапрос основной выборки, если возможно, использовать выражения EXISTS или NOT EXISTS . 

Для DML, можно рассмотреть возможность использования выражения FORALL, который значительно более быстрый, 
чем аналогичное выражение, выполненное внутри цикла.
...
Использование ключевых слов BULK COLLECT в выборках - очень эффективный способ получения результирующих данных. 
Вместо циклической обработки каждого ряда, мы сохраняем результат в одной или нескольких коллекциях, 
все это делается в рамках одной операцией. Это ключевое слово может использоваться совместно с выражениями 
SELECT INTO, FETCH INTO и RETURNING INTO .

При использовании ключевых слов BULK COLLECT все переменные в списке INTO должны быть коллекциями. 
Колонки таблицы могут быть, как скалярными значениями так и структурами, включая объектные типы.
...
Коллекции инициализируются автоматически. 
Вложенные таблицы и ассоциативные массивы расширяются для сохранения необходимого количества элементов. 

Если необходимо использовать массивы с фиксированным размером, тогда нужно убедиться, 
что декларируемый размер массива соответствует объемам выбираемых данных. 
Элементы вставляются в коллекции, начиная с индекса 1, при этом все существующие значения перезаписываются.

Т.к. обработка выражения BULK COLLECT INTO подобна циклу FETCH, то не генерируется исключение NO_DATA_FOUND, если не выбран ни один ряд. Если требуется, наличие выбранных данных надо проверять вручную.

Чтобы предотвратить переполнение памяти данными выборки, можено использовать выражение LIMIT или псевдоколонку ROWNUM 
для ограничения числа записей в выборке. Кроме того возможно использование выражения SAMPLE для получения набора случайных записей.
...
Также можно обрабатывать большие объемы результирующих данных, указав количество записей, которые будут выбраны из курсора за один раз.
...
Дополнительное условие LIMIT может использоваться только с выражением FETCH и ограничивает число рядов, 
выбираемых из баз данных. 
В следующем примере на каждой итерации цикла извлекается не больше десяти рядов и помещается в таблицу empids. 
Предыдущие значения перетираются. Обратите внимание на использование empids.count как условия выхода из цикла.
...
Можно использовать BULK COLLECT в условии RETURNING INTO выражений INSERT, UPDATE, DELETE.
...
Можно объединить условие BULK COLLECT и выражение FORALL . Результирующая коллекция будет заполнена 
итерациями выражения FORALL . 
В следующем примере для каждого удаленного ряда значение employee_id сохраняется в коллекцию e_ids. 
Коллекция depts хранит три элемента, таким образом выражение FORALL выполнит три итерации. 
Если каждый оператор DELTE выполненный выражением FORALL удалит пять рядов, то в результате коллекция e_ids, 
которая хранит значения из удаленных рядов, будет содержать 15 элементов.
...
Если бы использовался цикл FOR вместо выражения FORALL, 
то набор результирующих значений перетирался бы следующим выполнением выражения DELETE . 
